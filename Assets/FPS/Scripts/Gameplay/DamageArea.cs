...
public class DamageArea : MonoBehaviour
{
        ...
        QueryTriggerInteraction interaction, GameObject owner)
    {
        ...
        foreach (var coll in affectedColliders)
        {
            ...
            ScaleChange scaleChange = coll.GetComponent<ScaleChange>();
            if (scaleChange)
            {
                if (FindObjectOfType<PlayerWeaponsManager>().isShrinking)
                {
                    scaleChange.Shrink(FindObjectOfType<PlayerWeaponsManager>().GetActiveWeapon());
                }
                else if (FindObjectOfType<PlayerWeaponsManager>().isGrowing)
                {
                    scaleChange.Grow(FindObjectOfType<PlayerWeaponsManager>().GetActiveWeapon());
                }
            }
        }
